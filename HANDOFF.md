...
- **Los candados de rol los pone el servidor** (21 Sep 2026): las 53 escrituras que solo estaban protegidas por la pantalla ahora piden el módulo de esa pantalla, y las 32 lecturas sueltas piden staff **activo** — un alta recién creada, en espera de aprobación, ya no lee nada de la empresa. `moduleMiddleware` acepta varios módulos para lo que vive en dos pantallas (el SKU por contraparte, el destino del cliente). De paso, la pantalla dejó de ofrecer botones que el servidor rechaza: la liquidación, los gastos de la carga, el portal del proveedor, la nota de crédito, los adelantos y los estados financieros son de Finanzas y ahora se esconden —o dicen de qué módulo son— en lugar de tronar al guardar.
- **La fecha es la de Nogales, no la del servidor** (21 Sep 2026): `todayISO()` y los folios `LOT-AAMM-` / `CPO-AAMM-` salen de `America/Phoenix`. Antes, todo lo capturado después de las 5 de la tarde nacía fechado al día siguiente, y el 30 a las 6 el folio brincaba de mes. Servidor y navegador ya dan la misma fecha.
- **Antigüedad por fecha compromiso** (22 Sep 2026, migración 0049): la factura del proveedor se vencía a **7 días escritos a fuego** y la del cliente a **14** cuando no había plazo. Ahora el proveedor tiene su plazo default (en blanco hasta capturarlo), capturar su factura pide **fecha real y vencimiento**, el gasto acepta el suyo, y las tres pantallas —CxC, CxP y Gastos— leen **un solo cálculo** por fecha compromiso. **CxP estrenó su pestaña de antigüedad**, con vencido y sin plazo. La columna **"Sin plazo"** separa lo que falta capturar de lo que está al corriente. La ficha del proveedor sugiere su plazo sacándolo de **sus propias facturas del corte** — 53 de las 62 vencieron a 21 días — con un botón para aceptarlo; nunca se escribe solo.
- **Peso–dólar, parte A: moneda y tipo de cambio en las compras** (22 Sep 2026, migración 0050). Plein compra en pesos y lleva libros en dólares, y el ERP no tenía moneda en ningún documento de dinero. Ahora la **orden de compra** se captura en pesos con **un solo TC pactado por carga**; el costo del lote nace en dólares (÷ TC) con el original en pesos aparte; la **factura del proveedor** nace **congelada** en pesos a ese TC y suma en dólares; **gastos y adelantos** aceptan pesos con su TC (el gasto propone el TC de su carga); **moneda de pago default por proveedor**, editable en cada documento; el impreso de la orden y el portal del productor van en pesos. Reglas: sin TC un documento en pesos **no se guarda**; TC fuera de 5–50 se rechaza; con factura viva o liquidación emitida ni la moneda ni el TC se mueven; re-guardar el mismo original al mismo TC conserva el dólar guardado. **Nada de lo que suma cambió de unidad**: CxP, P&L, Balance, Chase y las anclas siguen en dólares. El motor es el de Azagro (`~/Documents/azagro/src/lib/erp/fx.ts`) **invertido** — allá los libros van en pesos. **Parte B construida el mismo día** (ver abajo).
- **Peso–dólar, parte B: el diferencial cambiario al pagar** (22 Sep 2026, migración 0051). Una factura de proveedor o un gasto pactados en pesos **se pagan en pesos**: se capturan los pesos que recibió el proveedor y el **TC al que convirtió el banco** (del estado de cuenta; nunca se precarga). Chase baja por lo que de verdad salió (`pesos / TC del banco`), la deuda por lo pactado (`pesos / TC pactado`), y la diferencia es **resultado cambiario de Plein** (`cash_movements.fx_result`, + ganancia) — nunca del productor, nunca por `expenses`. Entra al P&L en la cuenta **58100 "Resultado cambiario"** (reasignable en Cuentas, mapeo `fx_result`; escape 58100 → 58000 → 59999), recortado por la fecha del pago. El pago que completa los pesos cierra el documento en $0.00 exacto (los centavos van al resultado). Cancelar un pago y BORRAR devuelven **lo abonado** (`|amount| + fx_result`), no lo que salió de Chase. Un gasto pagado en pesos no se cancela sin cancelar antes su pago. Las facturas en dólares —incluidas las 62 del corte— no aceptan pesos. CxP tiene un panel de **exposición en pesos** con un TC de referencia que teclea Miguel y no se guarda. **Fuera de B:** la remisión al productor (comisión pura) no tiene TC pactado — se sigue pagando en dólares; una cuenta en pesos o pesos comprados por adelantado son la parte C.
- **Nómina** (22 Sep 2026, migración 0052). Camino propio, fuera de Gastos: **empleados** (personal, aparte de `staff`; catálogo, sobrevive a BORRAR; nace vacío — nombres, áreas y sueldos los captura Miguel), **periodos** `NOM-001` con del/al, y un **renglón por empleado** con bruto, deducciones y neto (nombre y partida del V8 congelados al capturar). Un periodo se captura en borrador y se **cierra**: entra al P&L por el **bruto**, por su **fecha de fin**, en la cuenta **52500 "Nómina"** (que ya existía desde el bloque 0 — `MODELO-NEGOCIO` decía que no; era texto viejo) vía el mapeo `payroll` de Cuentas (escape: concepto → `payroll` → partida → 52500 → cajón). Al cerrar, o después, se dice **cómo se pagó**: *por Chase* (un movimiento kind `nomina` por empleado, con la fecha real del banco, **nunca antes del corte** ni futura), *ya pagada y Chase ya lo refleja* (no mueve la caja: para lo capturado a mano en Tesorería — y si la fecha de pago es **anterior al corte**, el periodo queda marcado "antes del corte": es del V8, se conserva como registro y **no entra al P&L ni a los pasivos**, porque ese dinero ya vive en CORTE-CHASE), o *por pagar* (neto en la nueva cuenta de pasivo **20300 Nómina por pagar**). Las deducciones retenidas se acumulan en **20350 Retenciones de nómina**. Nada de esto toca la 20100 ni el KPI de CxP: el ancla no se mueve. Un movimiento de nómina **no se cancela desde Tesorería** (solo desde Nómina, que además regresa el periodo a por pagar); un periodo pagado por Chase no se cancela ni se reabre sin cancelar antes su pago; cancelar un periodo lo saca del P&L y del pasivo pero conserva sus renglones. BORRAR limpia periodos y renglones (antes que los movimientos) y deja a los empleados. La revisión adversarial (6 lentes, 3 escépticos por hallazgo) cerró además: un gasto con partida "Gasto Nómina" **se rechaza** (se contaría dos veces; el registro manual de Chase sí la acepta), el KPI "Pagos" de Tesorería suma la nómina, un movimiento cancelado ya no se ofrece para conciliar, un pago por Chase que truene a medias se limpia con "Cancelar pago" y no cobra dos veces al reintentar, cerrar un periodo con un empleado que ya cobró esos días en otro periodo pide confirmarlo como pago adicional, y dar de baja a un empleado no le borra la ficha. **Fuera del bloque:** enterar las retenciones (si se pagan como gasto normal, se cuentan dos veces — pendiente), nómina en pesos, y el prorrateo por carga (§ 2 de MODELO-NEGOCIO), que ya tiene de dónde leer: los periodos cerrados por fecha de fin.
- **Listo para operar** (23 Sep 2026, rama `listo-para-operar`, sin migración). **Gastos generales** (renta, cobros del banco, viáticos, asesoría…): al crear el gasto hay tres opciones — *por pagar*, **pagado desde Chase** (gasto + movimiento en Tesorería en un paso, con fecha del banco, método y referencia; nunca antes del corte) y *ya pagado sin mover Chase* (efectivo, tarjeta). La línea manual de Chase ya no acepta una salida con concepto de gasto (movía la caja sin tocar el P&L) y manda a Gastos o a Nómina. Cancelar un gasto "ya pagado" ya no le suma a Chase dinero que nunca salió. Hallazgos **21** (factura con adelanto recuperado se cancela y devuelve el adelanto), **26** (estado de cuenta con detalle y créditos), **42** (adelanto cancelado suelta su línea de banco), **54** (pedido en pesos no se convierte en venta en dólares) resueltos; **22** ya estaba resuelto (confirmado). **BORRAR se queda** a petición de Miguel: arranca a prueba y error. **Respaldo:** snapshot manual de Neon `antes-de-operar-2026-09-23` (`snap-lucky-feather-avq4htic`); no hay respaldo programado.
- **Financiamiento no es gasto** (23 Sep 2026, rama `financiamiento-no-es-gasto`, sin migración): el concepto "Financiamiento" del V8 es dinero prestado o aportado a productores; Gastos y Tesorería lo rechazan y mandan al adelanto al productor (Contactos → Proveedores → Cuenta corriente). La línea manual de Chase ya solo ofrece concepto en depósitos. **Decidido con Miguel:** corte nuevo al 30 Sep 2026 y arranque el 1 Oct (ver `MODELO-NEGOCIO.md` → Decisiones del 23 Sep 2026); la mensualidad de los socios es sueldo.
...
| directo a `main` | `bloque-0-cuentas-y-periodo` | **Bloque 0 — el P&L lee el catálogo de cuentas y acepta periodo.** Había **cuatro** listas de categorías de gasto que no coincidían: el catálogo real (`money_concepts`), el mapeo editable (`gl_mappings`, sembrado en inglés en `0009`), una lista escrita a mano en la pantalla de Cuentas, y un `switch` dentro de `getFinancials`. Casi todo el catálogo real de Miguel —Aduanas, In & out, Reempaque, Renta, Viáticos, Nómina— no estaba en ninguna de las tres últimas, así que caía en el cajón "59999 General", y **cada concepto nuevo nacía roto**. Ahora la cuenta se resuelve con cuatro escapes: la capturada en el gasto (`expenses.account_number`, viva por primera vez en 38 migraciones), el mapeo de su concepto, el de su **partida**, y el cajón. Las cinco partidas (Costo, Gasto de Venta, Gasto Nómina, Gasto Administrativo, Gasto Financiero) son las del libro V8 de Miguel y cubren el catálogo entero. El P&L avisa cuánto cayó al cajón. Y `getFinancials` acepta `{from, to}`: el campo "Period" de Reportes, que era un `<input type="date">` conectado a nada, filtra de verdad — **el P&L es del periodo, el Balance sigue siendo la foto de hoy** y la pantalla lo dice. |
...
Migración del bloque 0: `0048_cuentas_por_partida` — **solo datos**, no crea ni altera una sola tabla ni columna. Crea cuatro cuentas que faltaban (`52500 Nómina`, `56000 Gastos de venta`, `57000 Gastos administrativos`, `58000 Gastos financieros`), siembra las cinco partidas a su cuenta, y manda el cajón al final del P&L, que es donde va un cajón. Decisiones de Miguel: la partida **Costo** va a costo de lo vendido (así aduanas, in & out, reempaque y cartón restan **arriba**, junto al costo de la fruta, como en su V8); **Fletes** conserva su renglón propio en `51000`; y las cuatro partidas de gasto tienen cuenta propia, no "General". Verificado en Chrome contra base local limpia (28/28) y regresión completa de las diez suites anteriores en verde con las anclas intactas, más **revisión adversarial sobre el diff antes de publicar**: 9 defectos reales, todos corregidos.

Dos cosas que la prueba destapó y que la migración cierra:
- **"Materia prima" se manda al cajón a propósito.** El costo de la fruta en Cosecha viene de la **orden de compra** y ya llega al P&L por `lots.unit_cost`. Como ese concepto vive bajo la partida Costo, habría heredado `50000` y **contado la misma fruta dos veces, en silencio**. Mandarlo al cajón no es un mapeo correcto: es una alarma, y el aviso del P&L la señala sola. Ese concepto no debe usarse como gasto en Cosecha.
- **Cinco conceptos estaban clavados al cajón** por la siembra de `0009` (Servicios de inspección, Control de calidad, Advertising, Commissions and fees, Disposal fees), que en su momento apuntó a `59999` porque era la única cuenta que había. Como el mapeo por concepto gana sobre la partida, habrían seguido en el cajón aunque su partida ya tuviera casa. Se sueltan.

Y una regresión que la prueba atrapó antes de publicar: al quitar el `switch` se perdían **los alias en español que solo vivían ahí** — "Seguros", "Cuotas y suscripciones" y "Honorarios legales y profesionales" nunca estuvieron en `gl_mappings`, el código los reconocía a mano. Sin ellos habrían cambiado de cuenta en silencio. Quedan sembrados, que es donde debieron estar siempre, y desde hoy se mueven desde la pantalla.

**Lo que destapó la revisión adversarial, todo corregido y con prueba propia. El grave:** el P&L dejaba de **sumar consigo mismo**. Un gasto mapeado a una cuenta de costo —y con la partida Costo en `50000`, eso es aduanas, cartón, in & out, reempaque y comisión— se imprimía arriba, en el bloque de costo de lo vendido, pero **no entraba al total de ese bloque ni bajaba la utilidad bruta**. Los renglones no sumaban al subtotal impreso y la utilidad bruta salía inflada por exactamente ese monto (reproducido: renglón de costo $1,500, subtotal impreso $0). Ahora entran al total y se descuentan de lo que se resta abajo, así que la utilidad **neta no cambia** — y la prueba fija las dos igualdades: los renglones de costo suman al subtotal, y utilidad bruta menos los renglones de gasto da la utilidad neta.

**El segundo:** dos filas de `0009` que llevaban años **inertes** —`Cost of Labor → 50000` y `Equipment → 53000`— se habrían activado solas al encender el escape por concepto, y una de ellas cruza a costo, arriba de la utilidad bruta. Nadie decidió que la mano de obra fuera costo de lo vendido. Las dos se borran: `Cost of Labor` vuelve al cajón como estaba y `Equipment` sigue a su partida, igual que Renta o Asesoría.

**Los demás:** el formulario de gasto nuevo arrancaba en **"Materia prima"** —justo el concepto que no debe usarse— así que cada gasto guardado sin cambiarlo disparaba la alarma del cajón, y la pantalla de Cuentas ofrecía "sigue a su partida" para él, que era el camino a activar el doble conteo en silencio; el aviso del P&L decía algo falso en ese caso; la pantalla dejaba elegir cuentas de activo, pasivo e ingreso que la resolución descarta, guardando un mapeo que decía "Guardado" y no hacía nada; una cuenta de gasto con saldo inicial pasaba a mostrar cero; los conceptos se mostraban en inglés en una pantalla en español; cinco mapeos heredados quedaban sin ninguna pantalla donde editarlos (y el comentario de la migración prometía lo contrario); el botón de guardar había quedado en la otra tarjeta, de modo que editar las partidas y no volver perdía el cambio; y el periodo se colaba a la **Balanza de comprobación** sin verse ni poder quitarse desde ahí. La balanza ahora trae sus propios controles y avisa que con periodo activo **no cuadra, y es lo esperado**: recorta los renglones de resultados y no los de balance.
...
- **Nómina — hecha** (22 Sep 2026, migración 0052): ver arriba. Lo que sigue en el orden de `MODELO-NEGOCIO.md` es **indirectos por carga** (§ 2), que ya tiene la bolsa de nómina de dónde leer.
- **Bloque 0 — hecho** (19 Sep 2026): el P&L lee el catálogo de cuentas y acepta periodo. Era el
  requisito de cinco de los siete huecos de `MODELO-NEGOCIO.md`: sin él, peso-dólar, nómina, JEAM,
  indirectos y flete de salida caían todos en "59999 General". **Lo que sigue en ese orden es
  antigüedad por vencimiento** (hallazgo 51), que es chico y sin riesgo contable.
