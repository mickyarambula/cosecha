...
/** Aviso de error dentro del diálogo/panel: nunca se traga la falla. */
...
function Page() {
  ...
  async function createNew(e: React.FormEvent) {
    ...
    try {
      ...
      // Solo se limpia y se cierra cuando de verdad guardó.
      setNewName("");
      setOpen(false);
    } catch (e2) {
      setCreateErr(errorMessage(e2, "No se pudo crear el transportista."));
    } finally {
      setSaving(false);
    }
  }
    ...
    <div className="flex min-h-[calc(100dvh-7rem)]">
      <aside className="flex w-full max-w-xs shrink-0 flex-col border-r border-border bg-surface">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto">
              ...
              {rows.map((c) => (
                <button
                  ...
                  className={cn(
                    ...
                    sel === c.id
                      ? "bg-action/8 ring-1 ring-inset ring-action"
                      : "hover:bg-surface-2",
                  )}
                >
                  <span>{c.name}</span>
          ...
          {!carriers.loading && !groups.length ? (
            <p className="p-4 text-sm text-muted">Sin transportistas.</p>
          ) : null}
        </div>
      </aside>
      <section className="min-w-0 flex-1 overflow-y-auto p-5">
        {!current ? (
          <p className="pt-24 text-center text-muted">
            Selecciona un transportista para ver su detalle.
          </p>
        ) : (
          <div>
            <div className="mb-4 flex items-center justify-between">
              <h1 className="text-lg font-semibold">{current.name}</h1>
              <Badge tone={edit.is_active ? "ok" : "mute"}>
                {edit.is_active ? "Activo" : "Inactivo"}
              </Badge>
            </div>
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              ...
              <Field label="SCAC">
                <Input
                  value={edit.scac}
                  onChange={(e) => setEdit({ ...edit, scac: e.target.value })}
                  className="uppercase"
                />
              </Field>
              <Field label="CAAT">
                <Input
                  value={edit.caat}
                  onChange={(e) => setEdit({ ...edit, caat: e.target.value })}
                  className="uppercase"
                />
              </Field>
            ...
            <ErrorNote msg={saveErr} />
            <div className="mt-4 flex items-center justify-end gap-3">
              {savedAt && !saveErr ? <span className="text-sm text-ok">Guardado</span> : null}
          ...
          <form onSubmit={createNew}>
            <Field label="Línea transportista">
              <Input
                ...
                placeholder="Cornejos Trucking"
              />
            </Field>
            <p className="mt-1 text-xs text-muted">
              País, SCAC, CAAT y proveedor ligado se capturan en la ficha, al guardar este nombre.
            </p>
            <ErrorNote msg={createErr} />
...
function UnitsPanel({
  ...
  async function add(e: React.FormEvent) {
    ...
    try {
      ...
      setErr(errorMessage(e2, "No se pudo agregar la unidad."));
    } finally {
      setSaving(false);
    }
  }
  ...
  async function saveEdit(u: CarrierUnit) {
    ...
    try {
      ...
      setErr(errorMessage(e2, "No se pudo guardar la unidad."));
    } finally {
      setSaving(false);
    }
  }
  ...
  async function toggleActive(u: CarrierUnit) {
    ...
    try {
      ...
      setErr(errorMessage(e2, "No se pudo cambiar el estado de la unidad."));
    }
  }
  ...
  return (
    <div className="mt-6 border-t border-border pt-4">
      ...
      <form
        onSubmit={add}
        className="mb-3 grid gap-2 rounded-md border border-border p-3 sm:grid-cols-5"
      >
        <Select
        ...
        <Input
          ...
          onChange={(e) => setForm({ ...form, economic_number: e.target.value })}
        />
        <Input
            ...
            {units.map((u) =>
              editId === u.id ? (
                <tr key={u.id} className="border-t border-border">
                  ...
                  <td className="px-3 py-2">
                    <Input
                      value={editForm.economic_number}
                      onChange={(e) =>
                        setEditForm({ ...editForm, economic_number: e.target.value })
                      }
                    />
                  </td>
                  <td className="px-3 py-2">
                  ...
                  <td className="px-3 py-2">
                    <Input
                      ...
                      onChange={(e) => setEditForm({ ...editForm, model_year: e.target.value })}
                    />
                  </td>
                  <td className="px-3 py-2">
                    <Badge tone={u.is_active ? "ok" : "mute"}>
...
function DriversPanel({
  ...
  async function add(e: React.FormEvent) {
    ...
    try {
      ...
      setErr(errorMessage(e2, "No se pudo agregar el chofer."));
    } finally {
      setSaving(false);
    }
  }
  ...
  async function saveEdit(d: Driver) {
    ...
    try {
      ...
      setErr(errorMessage(e2, "No se pudo guardar el chofer."));
    } finally {
      setSaving(false);
    }
  }
  ...
  async function toggleActive(d: Driver) {
    ...
    try {
      ...
      setErr(errorMessage(e2, "No se pudo cambiar el estado del chofer."));
    }
  }
                ...
                <tr key={d.id} className="border-t border-border">
                  ...
                  <td className="px-3 py-2">
                    <Input
                      ...
                      onChange={(e) => setEditForm({ ...editForm, license_state: e.target.value })}
                    />
                  </td>
                  <td className="px-3 py-2">
                    <Input
                  ...
                  <td className="px-3 py-2">
                    <Badge tone={d.is_active ? "ok" : "mute"}>
